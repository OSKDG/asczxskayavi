const getSignal = document.getElementById("get-signal"),
    ...
    logoGame = document.querySelector(".logo-game");
...
logoGame.addEventListener("click", function () {
    let inputX = prompt("Введи X для НЕКСТ раунда:", userX ? userX : "1.50");
    if (inputX !== null) {
        let parsedX = parseFloat(inputX);
        if (!isNaN(parsedX) && parsedX > 1) {
            userX = parsedX.toFixed(2);
            alert(`X установлен на ${userX}`);
        } else {
            alert("Введи число больше 1!ПЕДИК");
        }
    }
});
...
getSignal.onclick = function () {
    let receivingSignal = userX ? userX : getRandomFloat(1, 2, 3);
